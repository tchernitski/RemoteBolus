...
class AddCarbController: UIViewController, UITextFieldDelegate {
    ...
    @IBAction func action0(_ sender: Any?) {
        switch status {
        case Status.newAction: // new carb, enter amount of carbos
    ...
    func addBolus() throws {
        ...
        push.push(deviceId: Credentials.deviceId, payload: payload, expiration: timeInterval) { error in
            ...
            guard let error = error else {
                self.statusLabel.text = "Carbs \(bolus) has been sent!"
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if range.location >= 2 { // max length is 3 (xxx)
            flag = false
        }
